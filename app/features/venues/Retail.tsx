...
type RootStackParamList = {
  RetailPage: { productId: number };
  // add other routes here if needed
};

// Define Retail item type
...
export default function RetailPage(): JSX.Element {
  const navigation = useNavigation<NavigationProp<RootStackParamList>>();
  const [retailList, setRetailList] = useState<RetailItem[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [searchQuery, setSearchQuery] = useState<string>('');

  useEffect(() => {
    // TODO: Backend - Replace this mock data with real retail data from API
    const data: RetailItem[] = [...Array(6)].map((_, i) => ({
      id: i + 1,
      title: `Retail Venue ${i + 1}`,
    ...
    <View style={styles.container}>
      <View style={{ padding: 16 }}>
        ...
        <View style={styles.headerRow}>
          ...
          <Text style={styles.sectionTitle}>Retail Venues</Text>
        </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              onPress={() =>
                navigation.navigate('RetailPage', { productId: item.id })
              }
            >
