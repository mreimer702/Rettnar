...
type RootStackParamList = {
  WorkspacesPage: { productId: number };
  // add other routes here if needed
};

// Define Workspaces item type
interface WorkspacesItem {
  id: number;
  title: string;
...
export default function WorkspacesPage(): JSX.Element {
  ...
  const [workspacesList, setWorkspacesList] = useState<WorkspacesItem[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [searchQuery, setSearchQuery] = useState<string>('');

  useEffect(() => {
    // TODO: Backend - Replace this mock data with real workspaces data from API
    const data: WorkspacesItem[] = [...Array(6)].map((_, i) => ({
      id: i + 1,
      title: `Workspace ${i + 1}`,
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              onPress={() =>
                navigation.navigate('WorkspacesPage', { productId: item.id })
              }
            >
